    ...
    try:
        ...
        for index, i in enumerate(lines):
            for n in i:
                ...
                line_dict[index] = [slope, y_intercept, [int(x1), min_y, int(x2), max_y]]
        ...
        for index in line_dict:
            ...
            # store the slope
            slope = line_dict[index][0]
            # store the intercept
            y_intercept = line_dict[index][1]
                ...
                for other_ms in final_lanes_copy:
                    ...
                    if not found_copy:
                        if abs(other_ms * 1.2) > abs(slope) > abs(other_ms * 0.8):
                            # loop until the slope is similar to the slope in the final_lanes array
        ...
        def average_lane(lane_data):
            ...
            for data in lane_data:
                ...
                x2s.append(data[2][2])
                y2s.append(data[2][3])
            # find the mean of the coordinates, and convert them to integers before returning
...
def process_img(image):
    ...
    # instantiate the slope variables
    slope1 = 0
    slope2 = 0
    try:
        # call the draw_lanes function over the original image, and store the returned values
        l1, l2, slope1, slope2 = draw_lanes(original_image, lines)
    ...
    try:
        ...
        pass
    # return the processed image, original image and both slope variables
    return processed_img, original_image, slope1, slope2

# functions to press the keys
def straight():
    press_key(W)
    release_key(A)
    release_key(D)
...
# countdown timer before it starts
...
def main():
    ...
    while(True):
        ...
        last_time = time.time()
        new_screen, original_image, slope1, slope2 = process_img(screen)
        # # displays the processed image
        # cv2.imshow('window', new_screen)
        # displays the original image
        cv2.imshow('window2', cv2.cvtColor(original_image, cv2.COLOR_BGR2RGB))

        # if statements that call the functions to press the keys depending on the value of slope1 and slope2
